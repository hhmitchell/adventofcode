...
def get_steps(vel_x, target):
    ...
    while vel[0] > 0 and pos[0] <= target.x1:
        step_probe(pos, vel)
        i += 1
...
def get_possible_y(target, step_num):
    ...
    for possible_y in range(target.y0, -target.y0):
        pos = [0, 0]
        vel = [0, possible_y]
        i = 0
        while pos[1] >= target.y0:
            step_probe(pos, vel)
            i += 1
            if target.in_y_range(pos[1]):
                if i not in possible_y_vel:
                    possible_y_vel[i] = []
                possible_y_vel[i].append(possible_y)
...
def main(args):
    ...
    for target in targets:
        ...
        print('possible x = {}'.format(possible_x))
        print('possible drop = {}'.format(possible_drop))
        print('possible num steps = {}'.format(possible_step_nums))

        possible_y = {}
        #for step_num in possible_step_nums:
        #    y_vels = get_possible_y(target, step_num)
        #    possible_y[step_num] = y_vels
        possible_y = get_possible_y(target, None)
        print('possible y = {}'.format(possible_y))
        ...
        if possible_drop:
            for x in possible_x:
                ...
                if dropping:
                    ...
                    all_drop_steps[steps[-1]] = drop_steps
                    print('drop_steps for step {} = {}'.format(
                        steps[-1], drop_steps))
                    print(max([z for (x, y, z) in drop_steps]))
                    max_y_vel = max([z for (x, y, z) in drop_steps])
                    height = get_max_y(max_y_vel)
                    if max_y is None or height > max_y:
                        max_y = height
        print('all drop steps = {}'.format(all_drop_steps))
        ...
        for x in possible_x:
            (steps, dropping) = possible_x[x]
            for step in steps:
                if step in possible_y:
                    for y in possible_y[step]:
                        possible_velos.add((x, y))
                    if dropping:
                        for y_step in possible_y:
                            if y_step > step:
                                for y in possible_y[y_step]:
                                    possible_velos.add((x, y))
            if dropping and steps[-1] in all_drop_steps:
                for drop_steps in all_drop_steps[steps[-1]]:
                    y = drop_steps[2]
                    possible_velos.add((x, y))

        print(len(possible_velos))
        lines = []
        line = []
        x = None
        for velo in sorted(possible_velos):
            if velo[0] != x:
                lines.append(', '.join(line))
                line = []
                x = velo[0]
            line.append('{},{}'.format(velo[0], velo[1]))
