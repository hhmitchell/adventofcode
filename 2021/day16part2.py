    ...
    def evaluate(self):
        if self.is_operator():
            if self.type == 0:
                ...
                return math.prod(
                    [packet.evaluate() for packet in self.packets])
            elif self.type == 2:
                ...
                if len(self.packets) != 2:
                    raise Exception(
                        'Packet of type {} contains {} sub-packets!'.format(
                            self.type, len(self.packets)))
                return 1 if self.packets[0].evaluate(
                ) > self.packets[1].evaluate() else 0
            elif self.type == 6:
                if len(self.packets) != 2:
                    raise Exception(
                        'Packet of type {} contains {} sub-packets!'.format(
...
def main(args):
    ...
    for digits in transmission:
        ...
        print('Evaluated value = {}'.format(packet.evaluate()))
