...
class Processor {
 public:
  ...
  void process_line(const std::string& line) {
    ...
    while (start_pos < line.size()) {
      ...
      if (itr != ordering_rules_.end()) {
        ...
        if (!intersection.empty()) {
          valid_rule = false;
          auto itr = fixed_updates.cbegin();
          itr += fixed_updates.size() - intersection.size();
          fixed_updates.insert(itr, page);

        } else {
          fixed_updates.push_back(page);
        }
      } else {
        fixed_updates.push_back(page);
      }
    ...
    if (!valid_rule) {
      sum_ += fixed_updates.at(fixed_updates.size() / 2);
    }
  }
